...
public class InventoryAdapter implements DestinyInventoryAdapter {
    ...
    @Override
    public Collection<Inventory> ofLegend(Legend anLegend) {
        Collection<Inventory> inventories = new ArrayList<>(anLegend.withCharacterIds().size() + 1);
        DestinyCredentials destinyCredentials = credentialsService.ofLegend(anLegend.withId());
        ...
        for (String characterId : anLegend.withCharacterIds()) {
            CharacterInventory bungieInventory = apiFacade
                    .getCharacterInventory((String.valueOf(anLegend.withId().withMembershipType())),
                            ...
                            characterId,
                            destinyCredentials.asCookieVal(),
                            destinyCredentials.xcsrf());
    ...
    @Override
    public boolean take(Item anItem, Inventory toCharacter) {
        return false;
    }

    @Override
    public boolean store(Item anItem, Inventory fromCharacter) {
        return false;
    }

    @Override
    public boolean equip(Item anItem, Inventory onCharacter) {
        return false;
    }
